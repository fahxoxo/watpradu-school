...
@section('content')
<h3>แก้ไขข้อมูลสถิติ นักเรียน - ปี {{ $studentStat->academic_year }}</h3>

<form action="{{ route('student-stats.update', $studentStat) }}" method="POST">
    @csrf
    @method('PUT')

    <div class="mb-3">
        <label class="form-label">ปีการศึกษา</label>
        <input type="text" name="academic_year" class="form-control" value="{{ old('academic_year', $studentStat->academic_year) }}" required>
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <div class="input-group mb-2">
                    ...
                    <input type="number" name="{{ $key }}_boys" class="form-control" value="{{ old($key.'_boys', $studentStat->{$key.'_boys'} ?? 0) }}" min="0">
                </div>
                <div class="input-group">
                    <span class="input-group-text">หญิง</span>
                    <input type="number" name="{{ $key }}_girls" class="form-control" value="{{ old($key.'_girls', $studentStat->{$key.'_girls'} ?? 0) }}" min="0">
                </div>
            </div>
            @endforeach
        </div>
